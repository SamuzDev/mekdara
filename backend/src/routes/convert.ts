  ...
  .post(
    ...
    async ({ body, set }) => {
      try {
        ...
        set.status = 400;
        return { error: getErrorMessage(err), _debug: err instanceof Error ? err.message : String(err) };
      }
    },
  ...
  .post(
    ...
    async ({ body, set }) => {
      try {
        ...
        set.status = 422;
        return { error: "Failed to process the HTML content", _debug: err instanceof Error ? err.message : String(err) };
      }
    },
