...
const createRays = (count: number, cycle: number): LightRay[] => {
  ...
  return Array.from({ length: count }, (_, i) => ({
    id: `${i}-${Math.round((8 + Math.random() * 84) * 10)}`,
...
function Ray({ left, rotate, width, swing, delay, duration, intensity }: LightRay) {
  return (
    <motion.div
      className="pointer-events-none absolute -top-[12%] h-[var(--light-rays-length)] w-[var(--ray-width)] origin-top -translate-x-1/2 rounded-full bg-linear-to-b from-[color-mix(in_srgb,var(--light-rays-color)_70%,transparent)] to-transparent opacity-0 mix-blend-screen blur-[var(--light-rays-blur)]"
